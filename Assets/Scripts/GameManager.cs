...
public class GameManager : MonoBehaviour {
    ...
    public void AssignBodyText(double isFractured, bool isElongated, double isProtruded, double isPerforated, double oilBoundry, double goldBoundry, int numberOfEthnicities, double oilDensity, double goldDensity, double woodDensity, double coastlinePercentage, double ethnicityPercentage, double irredentism) {
        ...
        if (isElongated) // check if state is elongated
            bodyTexts.Add("Elongated states will separate one side from another creating isolation of the two groups. Additionally from a military perspective, long state borders are exposed more and require military enforcement across borders. Isolation and differences add up as centrifugal forces and work to divide the state.");
        if (isProtruded > 0.40) // check if state is protruded
            bodyTexts.Add("A protrusion can cause unnecessary disputes over its territory. Additionally smooth communication may be disturbed. This can isolate the mainland and the protrusion. Isolation and differences add up as centrifugal forces and work to divide the state. (" + Mathf.Round((float)(isProtruded * 1000f)) / 10d + "% of your country is protruded)");
        if (isPerforated > 0) // check if state is perforated
            bodyTexts.Add("Perforated states have a break in their communication and transportation lines. The state would have to cooperate and manage the imports, exports and foreign businesses of the state it's perforating. (Perforated country is " + Mathf.Round((float)(isPerforated * 1000f)) / 10d + "% the size of your country)");
        if (oilBoundry > 0.1) // check if state has a resource crossing a border
            bodyTexts.Add("Oil crosses your state border. Valuable resources come with competition. States may have allocational boundary disputes to obtain as much of the resources. The dispute can harm relations between the two states and may reduce stability in the region. (" + Mathf.Round((float)(oilBoundry * 1000f)) / 10d + "% of your oil is shared)");
        if (oilDensity < .25)
        ...
        if (goldBoundry > 0.1) // check if state has a resource crossing a border
            bodyTexts.Add("Gold crosses your state border. Valuable resources come with competition. States may have allocational boundary disputes to obtain as much of the resources. The dispute can harm relations between the two states and may reduce stability in the region. (" + Mathf.Round((float)(goldBoundry * 1000f)) / 10d + "% of your gold is shared)");
        if (goldDensity < .25)
        ...
        if (numberOfEthnicities > 2)
            bodyTexts.Add("More than 3 nationalities support the growth of centrifugal forces. The cultural differences create division and ideas of self determination in one of those groups can lead to armed conflict for autonomy. Isolation and differences add up as centrifugal forces and work to divide the state. (There are " + numberOfEthnicities + " different nationalities in your country)");
